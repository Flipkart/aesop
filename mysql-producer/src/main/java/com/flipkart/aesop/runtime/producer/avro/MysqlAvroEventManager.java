...
public class MysqlAvroEventManager
{
	 ...
	 * Sample :  [header=BinlogEventV4HeaderImpl[timestamp=1394108600000,eventType=25,serverId=1,eventLength=85,nextPosition=1501,flags=0,timestampOfReceipt=1394108600580]
	 ...
	 */
	public List<DbChangeEntry> frameAvroRecord(final BinlogEventV4Header eventHeader, final List<Row> rowList, final DbusOpcode dbusOpCode,
			@SuppressWarnings("rawtypes") BinLogEventMapper binLogEventMapper, final Schema schema , final long scn){
		...
		try{
			...
			for(Row row: rowList){
				...
				GenericRecord genericRecord = binLogEventMapper.mapBinLogEvent(eventHeader,row,dbusOpCode);
				List<KeyPair> keyPairList = generateKeyPair(columns, schema);
				DbChangeEntry dbChangeEntry = new DbChangeEntry(scn, timestampInNanos, genericRecord, dbusOpCode , isReplicated, schema, keyPairList);
		...
		if (keyPairList.size() == 1) {
			...
			if (pKeyType == Schema.Type.INT){
				if (key instanceof Integer){
					...
					String message = "Schema.Type does not match actual key type (INT) "
				...
				if (key instanceof Long){
					...
					String message = "Schema.Type does not match actual key type (LONG) "
							+ key.getClass().getName();
					LOGGER.error(message);
					throw new DatabusException(message);
				}
