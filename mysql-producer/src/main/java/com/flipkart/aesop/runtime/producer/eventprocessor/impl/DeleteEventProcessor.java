...
/**
 * The <code>DeleteEventProcessor</code> processes DeleteRowsEvent from source. This event gets called when ever few row/(s) are deleted at the source.
 * @author Shoury B
...
public class DeleteEventProcessor implements BinLogEventProcessor{
	...
	public void process(BinlogEventV4 event, OpenReplicationListener listener) throws Exception {
		...
		listener.getMysqlTransactionManager().performChanges(deleteRowsEvent.getHeader(), deleteRowsEvent.getRows(), DbusOpcode.DELETE);
