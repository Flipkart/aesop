...
/**
 * <code>PersonBinaryLogEventMapper</code> is an implementation of the {@link BinLogEventMapper} that creates instance
 * of the type {@link Person} from Bin log events
 * @author Shoury B
...
public class PersonBinaryLogEventMapper implements BinLogEventMapper<com.flipkart.aesop.events.ortest.Person>
{
	...
	/**
	 * Interface method implementation. Creates {@link Person} instance based on data in {@link Row} ,
	 ...
	 */
	@Override
	public Person mapBinLogEvent(BinlogEventV4Header header, Row row, DbusOpcode databusCode, Schema schema)
	{
		List<Column> columns = row.getColumns();
		try
		{
			Person person =
			        Person.newBuilder().setKey((Long) orToAvroMapper.orToAvroType(columns.get(0)))
			                ...
			                .setDeleted((CharSequence) orToAvroMapper.orToAvroType(columns.get(4))).build();
